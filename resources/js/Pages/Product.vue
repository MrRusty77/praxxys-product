        ...
        <div class="w-full p-2 rounded-full">
                ...
                <table class="w-full bg-white border-none rounded table-auto border-spacing-3">
                    ...
                    <tbody>
                        <tr class="p-2 text-black border border-solid rounded hover:ring-2 hover:ring-blue-500 focus:ring-2 focus:ring-blue-500"
                            v-for="(value) in pagination.data" :key="value.hash">
            ...
            <div class="flex flex-none w-full p-2">
                ...
                <div class="flex-none float-right w-6/12">
                    <VuePaginationTw class="float-right" v-if="!processing" :totalItems="pagination.meta.total"
                        :currentPage="pagination.meta.current_page" :perPage="pagination.meta.per_page"
                        @pageChanged="search" :goButton="false" styled="centered" borderActiveColor="border-red-500"
    ...
    methods: {
        ...
        async addOrEdit( action, product ){
            ...
            if (action == 'Edit') {
                ...
                this.formData.images = await this.axios.post('/images/get', product)
                    .then(({ data }) => {
                        return data;
                    }).catch(({ response: { data } }) => {
                        ElMessage({
        ...
        async getProductImages( product ) {
            ...
            await this.axios.post('/images/get', product)
                .then(({ data }) => {
                    return data;
                }).catch(({ response: { data } }) => {
                    ElMessage({
        ...
        async photoOnChange(e) {
            ...
            this.formData.images.push({
                path: await this.axios.post('/product/uploadImg', Imagedata, config)
        ...
        async save( form ){
            this.saving = true;
            await this.axios.post( '/product/AddOrUpdate', form )
            .then( ({ data }) => {
        ...
        async remove(product) {
            ...
            await this.axios.post('/product/remove', product)
                .then(({ data }) => {
                    ElMessage({
                        type: 'success',
